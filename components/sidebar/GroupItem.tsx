...
export interface GroupItemProps {
  ...
  onClearDelete: () => void;
  onOpenProfile?: (id: string | number, name: string) => void;
}
...
const GroupItem = memo(function GroupItem({
  ...
  const unreadCount = useChatStore((s) => s.unread[sid] || 0);
  const lastPreview = useChatStore((s) => s.lastPreview[sid] || "");
  const mutedUntil = useContactStore((s) => s.mutedChats[sid]);
  ...
  return (
    <div
      ...
      onTouchStart={() => {
        longPressRef.current = setTimeout(() => onDeleteTarget(sid), 500);
      }}
      onTouchEnd={() => {
    ...
    >
      ...
      {isDeleteTarget && (
        <button className="sb-delete-btn" title="Delete group chat" aria-label="Delete group chat" onClick={(e) => { e.stopPropagation(); onDelete(g.id); }}>
