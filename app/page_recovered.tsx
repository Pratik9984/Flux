...
// ─── PULSE v5.7 — Call notification & ringtone reliability fixes ──────────────
...
const PulseNative = IS_NATIVE
  ? registerPlugin<PulseNativePlugin>("PulseNative")
  : null;
...
export default function PulseChat() {
  ...
  useEffect(() => {
    ...
    (window as any).__pulseReady = true;
  }, []);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("pulse_ringtone_name");
  ...
  const handleRingtoneChange = (name: string) => {
    ...
    try { localStorage.setItem("pulse_ringtone_name", name); } catch {}
    if (IS_NATIVE && PulseNative) {
      (PulseNative as any).setRingtone({ ringtone: name }).catch(() => {});
    }
  };
  ...
  const applyAudioOutput = useCallback((speaker: boolean) => {
    if (IS_NATIVE && PulseNative) {
        ...
        (PulseNative as any).setAudioMode({ speaker }).catch(() => {});
        return;
    }
    // Web fallback
  ...
  const _finalizeAuth = (accessToken: string, email: string) => {
    ...
    // Start native background service for notifications when app is closed
    if (PulseNative) {
      PulseNative.startService({ token: accessToken, wsUrl: API }).catch(() => { });
    }
  };
  ...
  const logout = () => {
    ...
    if (PulseNative) PulseNative.stopService().catch(() => { });
    ...
    try { sessionStorage.removeItem("pulse_seen_ids"); sessionStorage.removeItem("_pulse_call_offer"); } catch { }
    idbSet("chat_token", "");
  ...
  const endCall = useCallback((sendSignal = true, explicitStatus?: "completed" | "missed" | "rejected") => {
    ...
    if (IS_NATIVE && PulseNative) {
        (PulseNative as any).stopCallAudio().catch(() => {});
    }
    if (PulseNative) PulseNative.stopCall().catch(() => {});
    try { sessionStorage.removeItem("_pulse_call_offer"); } catch { }
    ...
    if (cp && callDirectionRef.current) {
      ...
      if (targetChatId) setMessages(prev => { const next = [...prev, callRecord]; messagesCache.current[targetChatId] = next; return next; });
  ...
  const initWS = useCallback(() => {
    ...
    wsRef.current = ws;
    ws.onopen = () => {
      wsRetryDelay.current = 800; setWsStatus("connected");
      ...
      if (IS_NATIVE && PulseNative) {
          (PulseNative as any).getPendingAccept()
              .then((res: { pending: boolean; offerData: string }) => {
                  if (!res.pending || !res.offerData) return;
                  // Don't accept if already in a call (pulseCallAction path beat us here)
    ...
    ws.onclose = () => {
      ...
      setTimeout(() => { wsRetryDelay.current = Math.min(wsRetryDelay.current * 1.5, 5000); setWsStatus("reconnecting"); initWSRef.current?.(); }, wsRetryDelay.current);
    };
    ...
    switch (data.type) {
      case "typing":
        ...
        case "call_offer": {
          ...
          try { sessionStorage.setItem("_pulse_call_offer", JSON.stringify(offerPayload)); } catch { }
          ...
          if (!existingContact || !existingContact.display_name) {
            apiFetch<Contact>(`/profile/${encodeURIComponent(callerEmail)}`)
              .then(prof => {
                ...
                // Update stored offer with richer name
                try {
                  const refreshed = { ...offerPayload, peerName: richName };
                  sessionStorage.setItem("_pulse_call_offer", JSON.stringify(refreshed));
                } catch { }
              })
              .catch(() => { });
          }
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
        ...
        try {
            ...
            try { sessionStorage.setItem("_pulse_call_offer", detail.offerData); } catch {}
        } catch (err) {
    ...
    window.addEventListener("pulseCallAction", handler);
    return () => window.removeEventListener("pulseCallAction", handler);
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    if (token) {
      (async () => {
        ...
        if (PulseNative) {
          PulseNative.startService({ token, wsUrl: API }).catch(() => { });
        }
      })();
    }
  ...
  const setupWebRTC = async (targetEmail: string) => {
    ...
    pc.ontrack = event => {
        ...
        if (IS_NATIVE && PulseNative) {
            (PulseNative as any).startCallAudio().catch(() => {});
        }
  ...
  const startCall = async (video = true) => {
    ...
    try {
      ...
      for (const target of targetIds) {
        ...
        wsSend(JSON.stringify({ type: "call_offer", target_user: target, sdp: offer, isVideo: video, sender_name: profile.displayName || profile.username || currentUser }));
      }
  ...
  const acceptCall = async () => {
    ...
    stopRingtone();
    cancelCallNotification();
    if (PulseNative) PulseNative.stopCall().catch(() => {}); // stop Java Ringtone
    ...
    try {
        ...
        if (IS_NATIVE && PulseNative) {
            await (PulseNative as any).startCallAudio().catch(() => {});
        }
  ...
  const toggleSpeaker = () => {
    ...
    setIsSpeaker(s);
    if (IS_NATIVE && PulseNative) {
        (PulseNative as any).setAudioMode({ speaker: s }).catch(() => {});
    } else {
        applyAudioOutput(s);
    }
  };
          ...
          <aside className="sidebar">
            ...
            {showMyProfileSettings && (
              <div className="my-profile-settings-panel" onClick={e => e.stopPropagation()}>
                ...
                <div className="settings-action-row" style={{ cursor: "default", flexDirection: "column", alignItems: "flex-start", gap: 8, borderTop: "1px solid var(--border)", paddingTop: 12, marginTop: 4 }}>
                  <div style={{ display: "flex", alignItems: "center", width: "100%", fontSize: 13, color: "var(--text-1)" }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: 8 }}><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>Call Ringtone
                  </div>
                  <select 
                    value={ringtonePref} 
                    onChange={e => handleRingtoneChange(e.target.value)}
                    className="sb-field" 
                    style={{ padding: "4px 8px", fontSize: 13, height: "auto", cursor: "pointer" }}
                  >
                    <option value="ringtone">Pulse Default</option>
